...
function getFormValues() {
  ...
  return {
    ...
    crm: data.get("crm") || "",
    crmOther: (data.get("crmOther") || "").trim(),
    meet: getChecked("meet"),
    meetOther: (data.get("meetOther") || "").trim(),
    dialer: getChecked("dialer"),
    dialerOther: (data.get("dialerOther") || "").trim(),
    otherInt: getChecked("otherInt"),
    otherIntOther: (data.get("otherIntOther") || "").trim()
  };
}
...
function buildTechSummary(v) {
  const crmMap = {
    ...
    other: v.crmOther || "Other CRM"
  };
  let crmLabel = "";
  if (v.crm) crmLabel = crmMap[v.crm] || v.crm;

  const mapVals = (arr, map, otherVal) =>
    (arr || []).map((val) => (val === "other" ? otherVal || "Other" : map[val] || val));

  const meetMap = { zoom: "Zoom", gmeet: "Google Meet", teams: "Microsoft Teams" };
  const dialerMap = {
    salesloft: "Salesloft",
    ringcentral: "RingCentral",
    openphone: "OpenPhone",
    "hubspot-dialer": "HubSpot Dialer"
  };
  const otherIntMap = { slack: "Slack", "linkedin-sn": "LinkedIn Sales Navigator" };

  const meetLabels = mapVals(v.meet, meetMap, v.meetOther);
  const dialerLabels = mapVals(v.dialer, dialerMap, v.dialerOther);
  const otherLabels = mapVals(v.otherInt, otherIntMap, v.otherIntOther);

  const parts = [];
  if (crmLabel) parts.push(`CRM: ${crmLabel}`);
  if (meetLabels.length) parts.push(`Meetings/Email: ${meetLabels.join(", ")}`);
  if (dialerLabels.length) parts.push(`Dialer/SEP: ${dialerLabels.join(", ")}`);
  if (otherLabels.length) parts.push(`Other: ${otherLabels.join(", ")}`);
  return parts.join(" · ");
}
...
function renderPlanMeta() {
  ...
  const productsLabel =
    (formValues.products || []).map((p) => PRODUCT_LABELS[p] || p).join(", ") || "N/A";
  const outcomesLabel =
...
function renderPlan() {
  ...
  selectedProducts.forEach((productKey) => {
    ...
    card.className = "card";
    let docs = (PRODUCT_DOCS[productKey] || "").trim();
    ...
    `;
    if (docs) {
      html += `<p class="helper">${docs}</p>`;
    }

    // Determine which call types exist for this SKU (excluding phase 0)
    ...
    // Get call ordering for this family / callType intersection
  ...
  // Wire up collapsible headers & status selects
  wireCollapsibles();
  wireStatusSelects();
}
...
function wireStatusSelects() {
  $all(".task-status select").forEach((sel) => {
    sel.addEventListener("change", () => {
      ...
      const val = sel.value;
      taskStatus[id] = val;
      renderDashboard(); // keep dashboard live
    });
  });
}
...
function computeProgress() {
  const result = {
    ...
    byProduct: {}, // key -> { complete, total }
    byCall: [] // { productKey, productLabel, callType, family, complete, total }
  };
  ...
  filteredTasks.forEach((t) => {
    const status = taskStatus[t.id] || "Not Started";
    if (status === "NA") return; // ignore NA in denominator
    ...
    // by call (per product family / call type)
    if (t.productKey) {
...
function renderDashboard() {
  ...
  Object.entries(progress.byProduct).forEach(([key, val]) => {
    const pct =
      val.total === 0 ? 0 : Math.round((val.complete / val.total) * 100);
  ...
  // Roadmap: show call-level progress
  const callLines = [];
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("toStep2").addEventListener("click", () => {
    ...
    // initialize status if not existing
    filteredTasks.forEach((t) => {
  ...
  document.getElementById("exportPlanMd").addEventListener("click", () => {
    if (!filteredTasks.length) return;
  ...
  document.getElementById("exportPlanHtml").addEventListener("click", () => {
    if (!filteredTasks.length) return;
  ...
  document.getElementById("exportPlanDoc").addEventListener("click", () => {
    if (!filteredTasks.length) return;
    const html = generatePlanHtml();
    const name =
